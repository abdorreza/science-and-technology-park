...
<body>
    <?php
        ...
        {
            case "admin_all":
                $sth = $conn->prepare("SELECT * FROM users WHERE user_type='nazer'".$filter1.$filter2);
                if ( $filter1!="" )
                {
                ...
                break;
            case "admin_state":
                $sth = $conn->prepare("SELECT * FROM users WHERE user_type='nazer' AND state_code=".$_COOKIE["state_code"]);
                break;
            default:
        ...
        if ($count<=0 )
        {
            echo "<div id='board-msg'>کارشناسی تعریف نشده است</div>";
        }
    ?>
        <!--<table class="data1">-->
        <table id='state-tbl'>
                    ...
                    foreach($result as $row)
                    {
                        echo"
                            <tr>
                                ...
                                <td width='10%'><img src='../images/delete.png' onclick='Del_Nazer(".$row["code"].")'></td>
                                <td width='10%'><img src='../images/edit.png' onclick='show_edit_nazer(".$row['code'].")'></td>
                            </tr>
                            ";
						$andis=$andis+1;
                        /*$col=$col+1;
                        ...
                        echo "<td>";
                            echo "<div class='cell''>";
                                ...
                                echo "<div class='op'>";
                                    ...
                                    echo "<img src='../images/delete.png' onclick='Del_Nazer(".$row["code"].")'>";
                                    echo "<img src='../images/edit.png' onclick='show_edit_nazer(".$row['code'].")'>";
                                echo "</div>";
                            echo "</div>";
                        echo "</td>";
                        if ( $col==5 ) 
