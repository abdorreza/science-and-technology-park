...
<body>
    <?php
        ...
        {
            case "admin_all":
                $sth = $conn->prepare("SELECT * FROM users WHERE user_type='admin'".$filter1.$filter2);
                if ( $filter1!="" )
                {
                ...
                break;
            case "admin_state":
                $sth = $conn->prepare("SELECT * FROM users WHERE user_type='admin' AND state_code=".$_COOKIE["state_code"]);
                break;
            default:
                $sth = $conn->prepare("SELECT * FROM users WHERE user_type='admin' AND center_code=".$_COOKIE["center_code"]);
        }
        $sth->execute();
        $count = $sth->rowCount();
        if ($count<=0 )
        {
            echo "<div id='board-msg'>مدیر مرکزی تعریف نشده است</div>";
        }
    ?>
        <!--<table class="data">-->
        <table id='state-tbl'>
